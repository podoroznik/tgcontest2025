...
public class StarGiftPatterns {
    ...
    public static void initForProfile(float newLongRangePatternsMultiplier) {
        profileCenter = new float[]{
                -0f, -73f, 28, .3654f, 1, 0, 0,
                -0f, 90f, 28, .2354f, 1, 0, 0,
                -100f, 10f, 28, .3654f, 3, 35, -0,
                100f, 10f, 28, .3654f, 3, -45, -30,

                -155f * newLongRangePatternsMultiplier, 12f * newLongRangePatternsMultiplier, 25, .2354f, 4, 0, 5 * newLongRangePatternsMultiplier,
                155f * newLongRangePatternsMultiplier, 12f * newLongRangePatternsMultiplier, 25, .2354f, 4, -65 * newLongRangePatternsMultiplier, 10 * newLongRangePatternsMultiplier,

                -65f, -35f, 28, .3654f, 1, 0, 0,
                -117f * newLongRangePatternsMultiplier, -53f * newLongRangePatternsMultiplier, 25, .2354f, 2, 0, 20 * newLongRangePatternsMultiplier,

                -47f * newLongRangePatternsMultiplier, -87f * newLongRangePatternsMultiplier, 25, .2354f, 3, -13 * newLongRangePatternsMultiplier, 25 * newLongRangePatternsMultiplier,


                65f, -35f, 28, .3654f, 1, 0, 0,
                117f * newLongRangePatternsMultiplier, -53f * newLongRangePatternsMultiplier, 25, .2354f, 2, 0, 20 * newLongRangePatternsMultiplier,

                47f * newLongRangePatternsMultiplier, -87f * newLongRangePatternsMultiplier, 25, .2354f, 3, 10 * newLongRangePatternsMultiplier, 0,

                -70f, 55f, 28, .3654f, 1, 0, 0,
                -122f * newLongRangePatternsMultiplier, 73f * newLongRangePatternsMultiplier, 25, .2354f, 2, 35 * newLongRangePatternsMultiplier, -25 * newLongRangePatternsMultiplier,

                -52f * newLongRangePatternsMultiplier, 107f * newLongRangePatternsMultiplier, 28, .2354f, 5, 20 * newLongRangePatternsMultiplier, -80 * newLongRangePatternsMultiplier,


                70f, 55f, 28, .3654f, 1, 0, 0,
                122f * newLongRangePatternsMultiplier, 73f * newLongRangePatternsMultiplier, 25, .2354f, 2, -25 * newLongRangePatternsMultiplier, -5 * newLongRangePatternsMultiplier,

                52f * newLongRangePatternsMultiplier, 107f * newLongRangePatternsMultiplier, 28, .2354f, 5, -15 * newLongRangePatternsMultiplier, -30 * newLongRangePatternsMultiplier,

        };
    }

    private static float[] profileCenter = new float[]{};
    ...
    private static final Interpolator giftDecelerateInterpolator = new DecelerateInterpolator(0.6f);

    private static final Interpolator fifthPriorityInterpolator = input -> {
        if (input < 0.05) return 0;
        if (input > 0.75) return 1;
        float a = (input - 0.05f) / 0.7f;
        return giftDecelerateInterpolator.getInterpolation(a);
    };

    private static final Interpolator fourthPriorityInterpolator = input -> {
        if (input < 0.17) return 0;
        if (input > 0.8) return 1;
        float a = (input - 0.17f) / 0.63f;
        return giftDecelerateInterpolator.getInterpolation(a);
    };

    private static final Interpolator thirdPriorityInterpolator = input -> {
        if (input < 0.2) return 0;
        if (input > 0.8) return 1;
        float a = (input - 0.2f) / 0.6f;
        return giftDecelerateInterpolator.getInterpolation(a);
    };

    private static final Interpolator secondPriorityInterpolator = input -> {
        if (input < 0.48) return 0;
        if (input > 0.92) return 1;
        float a = (input - 0.48f) / 0.44f;
        return giftDecelerateInterpolator.getInterpolation(a);
    };

    private static final Interpolator firstPriorityInterpolator = input -> {
        if (input < 0.6) return 0;
        float a = (input - 0.6f) / 0.4f;
        return giftDecelerateInterpolator.getInterpolation(a);
    };
    ...
    public static void drawProfileCenterPattern(Canvas canvas, Drawable pattern, float w, float h, float avatarCenterY, float progress2, float currentAvatarCenter, float alpha) {
        ...
        for (int i = 0; i < profileCenter.length; i += 7) {
            final float priority = profileCenter[i + 4];
            float progress = priority == 1 ? firstPriorityInterpolator.getInterpolation(progress2) : (priority == 2 ? secondPriorityInterpolator.getInterpolation(progress2) : (priority == 3 ? thirdPriorityInterpolator.getInterpolation(progress2) : (priority == 4 ? fourthPriorityInterpolator.getInterpolation(progress2) : fifthPriorityInterpolator.getInterpolation(progress2))));
            ...
            float startX = c - dpf2(size) / 2.0f;
            float startY = currentAvatarCenter;
            float endX1 = (int) (c + dpf2(x) - dpf2(size) / 2.0f);
            float endY1 = (avatarCenterY + dpf2(y) - dpf2(size) / 2.0f);
            float midX1 = lerp(endX1, startX, 0.2f) + dp(offsetX);
            float midY1 = endY1 + dp(offsetY);


            float currentX1 = (1 - progress) * (1 - progress) * startX
                    + 2 * (1 - progress) * progress * midX1
                    + progress * progress * endX1;
            float currentY1 = (1 - progress) * (1 - progress) * startY
                    + 2 * (1 - progress) * progress * midY1
                    + progress * progress * endY1;

            pattern.setBounds(
                    (int) currentX1,
                    (int) currentY1,
                    (int) (currentX1 + dpf2(size)),
                    (int) (currentY1 + dpf2(size))
            );
